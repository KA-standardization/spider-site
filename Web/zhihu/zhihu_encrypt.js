...
const { JSDOM } = jsdom;
const dom = new JSDOM(`<!DOCTYPE html><p>a<\p>`);
window = dom.window;
...
var __g = {
  x: function (e, t) {
    for (var n = [], r = e.length, a = 0; 0 < r; r -= 16) {
      for (
        var o = e.slice(16 * a, 16 * (a + 1)), i = new Array(16), c = 0;
        c < 16;
        c++
      )
        i[c] = o[c] ^ t[c];
      (t = __g.r(i)), (n = n.concat(t)), a++;
    }
    return n;
  },
  r: function (e) {
    ...
    for (var r = 0; r < 32; r++) {
      var a = G(n[r + 1] ^ n[r + 2] ^ n[r + 3] ^ h.zk[r]);
      n[r + 4] = n[r] ^ a;
    }
...
console.log(Jagd("08db532639276f64a3ef04a9e2a509a2"));
